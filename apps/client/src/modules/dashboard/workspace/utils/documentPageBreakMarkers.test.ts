...
describe("document page break markers", () => {
  ...
  it("ignores repeated table headers when anchoring a continued table", () => {
    const previewCanvas = document.createElement("div");
    previewCanvas.innerHTML = `
      <section data-document-page="1">
        <div data-document-block-root="monthly-table">
          <table>
            <thead><tr><th>Month</th><th>Billable days</th></tr></thead>
            <tbody><tr><td>December 2026</td><td>11</td></tr></tbody>
          </table>
        </div>
      </section>
      <section data-document-page="2">
        <div data-document-block-root="monthly-table-continuation">
          <table>
            <thead><tr><th>Month</th><th>Billable days</th></tr></thead>
            <tbody><tr><td>TOTAL</td><td>120</td></tr></tbody>
          </table>
        </div>
      </section>
    `;

    const [anchor] = getPageBreakAnchors(previewCanvas);

    expect(anchor?.currentPhrases[0]).toContain("2026");
    expect(anchor?.nextPhrases[0]).toContain("total");
    expect(anchor?.nextPhrases[0]).not.toContain("month");
  });

  it("anchors after the final row when a table fits on one preview page", () => {
    const previewCanvas = document.createElement("div");
    previewCanvas.innerHTML = `
      <section data-document-page="1">
        <div data-document-block-root="reserve-table">
          <table>
            <thead><tr><th>Month</th><th>Real reserve</th></tr></thead>
            <tbody>
              <tr><td>December 2026</td><td>€924.00</td></tr>
              <tr><td>TOTAL</td><td>€10,080.00</td></tr>
            </tbody>
          </table>
        </div>
      </section>
      <section data-document-page="2">
        <div data-document-block-root="estimated-callout">
          <aside><p>Estimated acconti generated by the complete contract model.</p></aside>
        </div>
      </section>
    `;
    ...
    expect(anchor?.currentPhrases[0]).toContain("total");
    expect(anchor?.currentPhrases[0]).not.toContain("december");
    expect(anchor?.nextPhrases[0]).toContain("estimated");
  });
