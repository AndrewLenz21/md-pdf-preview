...
export function DocumentSourceToggle({
  ...
  return (
    <button
      ...
      onClick={() => onChange(source === "local" ? "cloud" : "local")}
    >
      <span
        aria-hidden="true"
      ...
      <span
        className={`relative z-10 flex items-center justify-center gap-1.5 rounded-md px-2 ${source === "cloud" ? "text-accent-foreground" : "text-muted-foreground"}`}
      >
